...
function Customizados() {
  ...
  useEffect(() => {
    const readExcelFile = async () => {
      ...
      const customizadoData = jsonData.filter(item => item.Ámbito && item.Ámbito.toLowerCase().includes('customizado'));
      setData(customizadoData);
      setFilteredData(customizadoData);
    };
  ...
  const columns = [
    {
      ...
      sortable: true,
      minWidth: '200px', // Establece un ancho mínimo para la columna
    },
    {
      name: 'Subcarpeta',
      ...
      minWidth: '200px', // Establece un ancho mínimo para la columna
      cell: row => (
        <div style={{ whiteSpace: 'normal', wordWrap: 'break-word' }}>{row.Subcarpeta}</div>
      ),
    },
    {
  ...
  return (
    <Container>
      <h1>Buscar un documento customizado</h1>
      <input
        type="text"
